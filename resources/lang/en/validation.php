<?php

declare(strict_types=1);

return [
    'failed' => 'The given data was invalid.',
    'required' => 'The :attribute field is required.',
    'email' => 'The :attribute must be a valid email address.',
    'confirmed' => 'The :attribute confirmation does not match.',
    'current_password' => 'The password is incorrect.',
    'string' => 'The :attribute must be a string.',
    'max' => [
        'string' => 'The :attribute may not be greater than :max characters.',
        'array' => 'The :attribute may not have more than :max items.',
        'file' => 'The :attribute may not be greater than :max kilobytes.',
    ],
    'min' => [
        'numeric' => 'The :attribute must be at least :min.',
        'string' => 'The :attribute must be at least :min characters.',
        'array' => 'The :attribute must have at least :min items.',
    ],
    'unique' => 'The :attribute has already been taken.',
    'password' => 'The password is incorrect.',
    'custom' => [
        'required' => 'The :attribute field is required.',
        'required_without' => 'The :attribute field is required when :values is not present.',
        'exists' => 'The selected :attribute is invalid.',
        'in' => 'The selected :attribute is invalid.',
        'max' => 'The :attribute may not be greater than :max.',
        'numeric' => 'The :attribute must be a number.',
        'min' => 'The :attribute must be at least :min.',
        'date' => 'The :attribute must be a valid date.',
        'after' => 'The :attribute must be a date after :date.',
        'uuid' => 'The :attribute must be a valid UUID.',
        'email' => 'The :attribute must be a valid email address.',
        'string' => 'The :attribute must be a string.',
        'array' => 'The :attribute must be an array.',
        'integer' => 'The :attribute must be an integer.',
        'file' => 'The :attribute must be a valid file.',
        'mimes' => 'The :attribute has an invalid file type. Allowed types: :values.',
        'mimetypes' => 'The :attribute has an unsupported file format.',
    ],
    'attributes' => [
        'assigned_to' => 'assigned employee',
        'attachable_id' => 'entity ID',
        'attachable_type' => 'entity type',
        'authorized_service_ids' => 'authorized services',
        'comment' => 'comment',
        'completed_service_ids' => 'completed services',
        'created_date' => 'creation date',
        'customer_id' => 'customer',
        'descriptions' => 'descriptions',
        'description' => 'description',
        'down_payment' => 'down payment',
        'estimated_completion' => 'estimated completion date',
        'email' => 'email address',
        'field_changed' => 'changed field',
        'file' => 'file',
        'files' => 'files',
        'device_name' => 'device name',
        'first_name' => 'first name',
        'item_type' => 'item type',
        'items' => 'items',
        'items.*.components.*' => 'component',
        'items.*.item_type' => 'item type',
        'locale' => 'language',
        'motor_info.brand' => 'motor brand',
        'motor_info.cylinder_count' => 'cylinder count',
        'motor_info.down_payment' => 'down payment',
        'motor_info.liters' => 'motor liters',
        'motor_info.model' => 'motor model',
        'motor_info.year' => 'motor year',
        'name' => 'file name',
        'names' => 'file names',
        'notes' => 'notes',
        'order_id' => 'order',
        'password' => 'password',
        'password_confirmation' => 'password confirmation',
        'priority' => 'priority',
        'service_key' => 'service',
        'services' => 'services',
        'services.*.order_item_id' => 'order item',
        'services.*.service_key' => 'service',
        'status' => 'status',
        'title' => 'title',
        'last_name' => 'last name',
        'uuid' => 'order UUID',
    ],
];
