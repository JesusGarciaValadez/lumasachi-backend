...
test('super administrator can create a user with the administration contract', function (): void {
    ...
    $response = $this->actingAs($superAdministrator)->post('/user', userAdministrationCrudValidPayload($company));
...
test('create validation preserves safe values and rejects duplicate email', function (): void {
    $superAdministrator = User::factory()->create([
        'is_active' => true,
        'role' => UserRole::SUPER_ADMINISTRATOR->value,
    ]);
...
test('administrator cannot submit creation or privileged fields', function (): void {
    ...
    $this->actingAs($administrator)
        ->post('/user', userAdministrationCrudValidPayload())
        ->assertForbidden();
});

test('index returns scoped dedicated paginated payload', function (): void {
    $company = Company::factory()->create();
...
test('update preserves password when omitted and changes it when supplied', function (): void {
    $superAdministrator = User::factory()->create([
        'is_active' => true,
        'role' => UserRole::SUPER_ADMINISTRATOR->value,
    ]);
...
test('forbidden administrator update fields are not accepted', function (): void {
    $company = Company::factory()->create();
    $administrator = User::factory()->create([
        'company_id' => $company->id,
        'is_active' => true,
 ...
 */
function userAdministrationCrudValidPayload(?Company $company = null): array
{
    return [
        'company_id' => $company?->id,
